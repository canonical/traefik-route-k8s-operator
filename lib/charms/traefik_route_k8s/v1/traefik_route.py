...
class TraefikRouteRequirer(Object):
    ...
    def __init__(self, charm: CharmBase, endpoint_name: str = "traefik-route"):
        super(TraefikRouteRequirer, self).__init__(charm, endpoint_name)
        self._stored.set_default(external_host=None, scheme=None)

        self._charm = charm
        self._relation = self.model.get_relation(endpoint_name)

        self.framework.observe(
            self._charm.on[endpoint_name].relation_changed, self._on_relation_changed
        )
        self.framework.observe(
            self._charm.on[endpoint_name].relation_broken, self._on_relation_broken
        )
    ...
    def submit_to_traefik(self, config):
        ...
        Caller is responsible for checking if self.is_ready first.
        """
        if not self._charm.unit.is_leader():
